...
const newQuote = () => {
    ...
    .then((data) => {
        ...
        let locationUri = encodeURI(document.location.href);
        let quoteUri = encodeURI(quote.innerText);
        let authorUri = encodeURI(author.innerText)

        twitter.setAttribute('href', `https://twitter.com/share?url=${locationUri}&text=${quoteUri}${authorUri}`);
    });
}
